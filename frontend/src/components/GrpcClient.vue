  ...
  export default {
    ...
    computed: {
      formattedResponse() {
        ...
        try {
          return JSON.stringify(JSON.parse(this.response), null, 2);  // Pretty-print JSON
        } catch (e) {
    ...
    methods: {
      getInvoice() {
        ...
        if (err) {
            ...
            this.response = response.getInvoicejson();  // Correct getter method based on generated code
        }
        });
